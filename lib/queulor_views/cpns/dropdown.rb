  ...
  class Dropdown < ViewComponent::Base
    ...
    erb_template <<-ERB
      <div class="queulor_dropdown" data-controller="queulor-views--queulor-dropdown">
        ...
        <div class="queulor_dropdown__content" data-queulor-views--queulor-dropdown-target="content" data-queulor-views--queulor-dropdown-show-class="<%= opt[:data-class] ?? 'show' %>">
          <% items.each do |item| %>
            <%= line_for item %>
          <% end %>
        </div>
